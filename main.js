// document.write("<script type='text/javascript' src='SceneManager.js'></script>");
...
function init(){
  ...
  // routeInfo.startGrid.x = 1;
  // routeInfo.startGrid.y = 6;
  // routeInfo.route.push(Direction8.TENKEY_2);
  // routeInfo.route.push(Direction8.TENKEY_2);
  // routeInfo.route.push(Direction8.TENKEY_6);
  // routeInfo.route.push(Direction8.TENKEY_8);
  // routeInfo.route.push(Direction8.TENKEY_4);
  // routeInfo.route.push(Direction8.TENKEY_4);
  // drawRoute(canvas, routeInfo);
  // return;
  // URLパラメータ取得
  var arg = new Object;
  ...
  document.getElementById("dropsCanvas").height = BALL_SIZE * 1; //BALL_SIZE * 2
  // フィールド
...
function saveLayout(){
  fieldScene.saveLayout();
  alert("Saved the movement");
}
function loadLayout(){
  if(fieldScene.lastLayout){
    fieldScene.reloadByLayout(fieldScene.lastLayout);
  }else{
    alert("Does not have saved located");
  }
}
function resetMove(){
  fieldScene.setStrategy(new FieldStrategyDropMove(fieldScene, false));
...
function play(){
  ...
  if(fieldScene.lastRoute){
    ...
    alert("Does not have saved operation");
  }
}
function stop(){
  if(fieldScene.lastLayout){
...
function updateInfo(){
  ...
  infoElement.innerHTML = "Combo:" + comboNum + " Move:" + moveNum;
