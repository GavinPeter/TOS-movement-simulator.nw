...
var FieldStrategyDropDelete = function(parent, fallNewDrop, recordPlay){
  ...
  this.updateTryDelete = function(){
    ...
    try{
      ...
      // console.log("checkBalls.length = " + checkBalls.length);
...
var FieldStrategyDropMove = function(parent, recordPlay){
  ...
  this.updateRecordPlayMouseInfo = function(){
    ...
    if(self.recordPlayFrameCount >= 24){
      ...
      // console.log("frame:" + self.recordPlayFrameCount + " dir:" + direction8);
    }
  ...
  this.updateMoving = function(){
    ...
    try{
      ...
      // マウスが動いたら
      if(mouseMoved){
        // console.log("mouse moved (" + mouseInfo.point.x + "," + mouseInfo.point.y + ")");
        // マウス移動の差分だけボールを移動
...
//           console.log("angle="+angle+" angleIsSlant="+angleIsSlant+" slantMove="+slantMove);
      ...
      if(mouseReleased){
        ...
        // var nextFieldMode = (tagetBall !== self.movingInfo.movingBall) ? FieldMode.TRY_DELETE : FieldMode.WAITING;
        parent.setBallAtGridPoint(parent.movingBall, gridPoint);
        parent.movingBall.setState(BallState.NORMAL);
        self.movingInfo = null;
        parent.movingBall = null;
        // self.mode = nextFieldMode;
        self.mode = Mode.WAITING;
        self.modeFrameCount = 0;
        // self.combos.length = 0;
...
var Combo = function(count, point, startFrame){
    ...
    this.draw = function(canvas){
        try{
            ...
            //                 ctx.fillText(self.count + " Combo", self.point.x, self.point.y);
            ctx.save();
            ...
            //     ctx.strokeText(self.count + " Combo", self.point.x, self.point.y);
            ctx.restore();
        }catch(e){
            debugTrace(e);
        }
